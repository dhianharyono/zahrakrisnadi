...
export default function AdminTestimonials() {
    ...
    const handleSaveTestimonial = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body = editingId ? { id: editingId, ...newTestimonial } : newTestimonial;
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 tracking-tight">Daftar Testimonial</h1>
                    <p className="text-gray-500 mt-1 text-sm">Kelola testimoni dari pasien</p>
                </div>
                <button
                    onClick={openAddModal}
                    className="bg-gradient-to-r from-primary to-orange-600 text-white px-6 py-3 rounded-2xl font-bold hover:shadow-lg hover:shadow-orange-200 hover:-translate-y-0.5 transition-all flex items-center gap-2 cursor-pointer"
                >
                    + Tambah Testimonial
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-orange-50/50 border-b border-orange-100">
                            <tr>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider">Nama</th>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider">Role</th>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider">Program</th>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider">Rating</th>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider">Pesan</th>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider">Status</th>
                                <th className="px-8 py-5 font-bold text-gray-600 text-xs uppercase tracking-wider text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {testimonials.length > 0 ? testimonials.map((item) => (
                                <tr key={item._id} className="hover:bg-white/80 transition-all duration-200 group">
                                    <td className="px-8 py-5 text-sm font-semibold text-gray-800 group-hover:text-primary transition-colors">{item.patientName}</td>
                                    <td className="px-8 py-5 text-sm text-gray-600">{item.role}</td>
                                    ...
                                    <td className="px-8 py-5 text-sm text-gray-600 truncate max-w-[200px] italic">"{item.message}"</td>
                                    <td className="px-8 py-5 text-sm">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${item.isVisible ? 'bg-green-100 text-green-700 border-green-200' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>
            ...
            {isAddModalOpen && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
                    <div className="bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl shadow-orange-100/50 border border-white/50 w-full max-w-lg overflow-hidden flex flex-col transform transition-all scale-100 animate-slide-up max-h-[90vh]">
                        <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-orange-50/80 to-white">
                            <h3 className="text-xl font-bold font-serif text-gray-800">{editingId ? 'Edit Testimonial' : 'Tambah Testimonial'}</h3>
                        </div>

                        <div className="p-6 overflow-y-auto">
                            <form onSubmit={handleSaveTestimonial} className="space-y-5">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Nama Pasien</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Role / Pekerjaan</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Pesan Testimoni</label>
                                    <textarea
                                        required
                                        rows={4}
            ...
            {/* Detail Modal */}
            {selectedTestimonial && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
                    <div className="bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl shadow-orange-100/50 border border-white/50 w-full max-w-lg overflow-hidden flex flex-col transform transition-all scale-100 animate-slide-up relative">
                        <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-orange-50/80 to-white flex justify-between items-center">
                            <h3 className="text-xl font-serif font-bold text-gray-800">Detail Testimonial</h3>
                            <button
                        ...
                        <div className="p-8 space-y-6">
                            <div className="flex items-center gap-4">
                                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary to-orange-400 flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-orange-200">
                                    {selectedTestimonial.patientName.charAt(0)}
                                </div>
                                <div>
                                    <h4 className="text-xl font-bold text-gray-800">{selectedTestimonial.patientName}</h4>
                                    <p className="text-gray-500 text-sm">{selectedTestimonial.role || 'Pasien'}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Program</span>
                                    <span className="font-semibold text-primary">{selectedTestimonial.program}</span>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Rating</span>
                            ...
                            <div className="bg-orange-50/50 p-6 rounded-2xl border border-orange-100 relative">
                                <span className="absolute top-4 left-4 text-4xl text-orange-200 font-serif leading-none">"</span>
                                <p className="text-gray-700 italic relative z-10 px-4 text-center leading-relaxed">
                                    {selectedTestimonial.message}
                                </p>
                                <span className="absolute bottom-[-10px] right-4 text-4xl text-orange-200 font-serif leading-none transform rotate-180">"</span>
                            </div>

                            <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                                <div className="flex flex-col">
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Status</span>
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold border w-fit ${selectedTestimonial.isVisible ? 'bg-green-100 text-green-700 border-green-200' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                                        {selectedTestimonial.isVisible ? "Ditampilkan" : "Disembunyikan"}
                                    </span>
                                </div>
                                <div className="flex flex-col items-end">
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Tanggal</span>
                                    <span className="text-sm font-medium text-gray-600">
                                        {new Date(selectedTestimonial.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                                    </span>
                                </div>
                            </div>
                        </div>
