    ...
    try {
      ...
      const body = {
        ...
        features: newPackage.features.map((f) => ({
          name: f.name,
          value:
            f.value === 'true' ? true : f.value === 'false' ? false : f.value,
        })),
      };
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {packages.length > 0 ? (
          packages.map((pkg) => (
            <div
              key={pkg._id}
              className={`bg-white rounded-3xl p-6 relative flex flex-col transition-all duration-300 border ${pkg.highlight ? 'border-orange-300 shadow-xl shadow-orange-100' : 'border-gray-100 shadow-sm hover:shadow-md'}`}
            >
              {pkg.highlight && (
            ...
            <div className='bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl shadow-orange-100/50 border border-white/50 w-full max-w-2xl overflow-hidden flex flex-col transform transition-all scale-100 animate-slide-up max-h-[95vh]'>
              ...
              <div className='p-6 overflow-y-auto'>
                <form onSubmit={handleSavePackage} className='space-y-6'>
                  ...
                  <div
                    className='p-4 bg-orange-50/50 rounded-2xl border border-orange-100 flex items-center justify-between cursor-pointer'
                    onClick={() =>
                      setNewPackage((prev) => ({
                        ...prev,
                  ...
                  >
                    <div>
                      ...
                      <p className='text-xs text-gray-500'>
                        Buat paket ini lebih menonjol dengan label
                        &quot;Popular&quot;.
                      </p>
                    </div>
                    <div
                      className={`w-12 h-6 rounded-full transition-colors relative ${newPackage.highlight ? 'bg-primary' : 'bg-gray-300'}`}
                    >
                      <div
                        className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-transform ${newPackage.highlight ? 'left-7' : 'left-1'}`}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className='border-t border-gray-100 pt-6'>
                    <div className='flex justify-between items-center mb-4'>
                      ...
                      <button
                        type='button'
                        onClick={addFeature}
                        className='text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-lg flex items-center gap-1 font-bold hover:bg-gray-200'
                      >
                    ...
                    <div className='space-y-3'>
                      {newPackage.features.map((feature, idx) => (
                        <div
                          key={idx}
                          className='flex items-start gap-3 bg-gray-50 p-2 pl-4 rounded-xl border border-gray-200'
                        >
                          <div className='flex-1 space-y-2 py-1'>
                            <input
                              type='text'
                              placeholder='Nama Fitur (cth: Konseling 1 on 1)'
                              required
                              value={feature.name}
                              onChange={(e) =>
