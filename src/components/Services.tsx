    ...
    >
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          ...
          {SERVICES_DATA.map((service, idx) => (
            <motion.div
              ...
              className={`group relative rounded-3xl p-6 sm:p-7 transition-all duration-300 flex flex-col justify-between ${service.highlight
                ? 'bg-linear-to-b from-amber-50/70 via-white to-white ring-2 ring-amber-500 shadow-md'
        ...
        <div className='md:hidden'>
          <div className='flex gap-4 overflow-x-auto pb-4 no-scrollbar items-stretch snap-x snap-mandatory px-2'>
            {SERVICES_DATA.map((service, idx) => (
              <div
                key={idx}
                className={`shrink-0 w-[85vw] rounded-3xl p-6 flex flex-col justify-between snap-center ${service.highlight
                  ? 'bg-linear-to-b from-amber-50/70 to-white ring-2 ring-amber-500 shadow-md'
                  : 'bg-white border border-slate-200/80'
                  }`}
              >
                <div>
                  <div className='relative h-44 w-full rounded-2xl overflow-hidden mb-4 bg-slate-100 ring-1 ring-slate-200/60 shadow-xs'>
                    <Image
                      src={service.image}
                      alt={service.title}
                      fill
        ...
        {showPricing && (
          <div
            id='consultation'
            className='py-12 mt-16 sm:mt-24 border-t border-slate-200/60'
          >
            <motion.div
              initial='hidden'
              whileInView='visible'
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div
                    ...
                    className='flex gap-6 lg:gap-8 overflow-x-auto no-scrollbar scroll-smooth py-4 items-stretch'
                  >
                    {pricingPlans.map((plan) => {
                      ...
                      return (
                        <div
                          key={plan._id}
                          className={`shrink-0 w-full sm:w-[320px] md:w-[360px] rounded-3xl p-6 md:p-8 transition-all duration-300 flex flex-col relative ${isHighlight
                            ? 'bg-linear-to-b from-amber-50/80 via-white to-white text-slate-900 ring-2 ring-amber-500 shadow-md'
                            : 'bg-white text-slate-900 border border-slate-200/80 shadow-xs hover:shadow-md'
                            }`}
                        >
                          {isHighlight && (
                            <div className='absolute top-0 right-0 bg-amber-500 text-white text-[11px] font-bold px-4 py-1.5 rounded-bl-2xl rounded-tr-3xl shadow-xs uppercase tracking-wider'>
                              POPULAR
                            </div>
                          )}
                          ...
                          <div className='mb-6 border-b border-slate-200/80 pb-6 flex items-baseline gap-1.5'>
                          ...
                          <ul className='space-y-3.5 mb-8 grow'>
                            {plan.features.map((feature, idx) => (
                              <li key={idx} className='flex items-start gap-3'>
                                <div className='mt-0.5 w-5 h-5 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center shrink-0'>
                          ...
                          <button
                            ...
                            className={`w-full py-3.5 rounded-full font-bold flex items-center justify-center transition-all cursor-pointer text-sm md:text-base mt-auto shadow-xs hover:shadow-md ${isHighlight
                              ? 'bg-amber-500 hover:bg-amber-600 text-white'
                              : 'bg-slate-100 hover:bg-amber-50 hover:text-amber-600 text-slate-800 border border-slate-200/80'
                              }`}
                          >
