            ...
            {PRICING_PLANS.map((plan, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className='mb-6'>
                    <h3 className={`text-2xl font-bold mb-2 ${isHighlight ? 'text-white' : 'text-gray-900'}`}>
                      {plan.name}
                    </h3>
                  ...
                  <div className='mb-8 border-b border-gray-200/20 pb-8'>
                    <span className={`text-4xl font-extrabold block ${isHighlight ? 'text-white' : 'text-gray-900'}`}>
                      {plan.price}
                    </span>
                    <span className={`text-sm font-medium ${isHighlight ? 'text-orange-400' : 'text-primary'}`}>
                      / {plan.duration}
                    </span>
                  </div>
                  ...
                  <ul className='space-y-4 mb-8 grow'>
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className='flex items-start gap-3'>
                        <div className={`mt-0.5 min-w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${isHighlight ? 'bg-orange-500/20 text-orange-400' : 'bg-green-100 text-green-600'}`}>
                          <Check size={12} strokeWidth={3} />
                        </div>
                        <div className='text-sm'>
                          <span className={`font-medium block ${isHighlight ? 'text-gray-200' : 'text-gray-800'}`}>
                            {feature.name}
                          </span>
                          {feature.value !== true && (
                            <span className={`text-xs block mt-0.5 ${isHighlight ? 'text-gray-400' : 'text-gray-500'}`}>
                              {feature.value}
                            </span>
                          )}
                        </div>
              ...
              {PRICING_PLANS.map((plan, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className='mb-4'>
                      <h3 className={`text-xl font-bold mb-1.5 ${isHighlight ? 'text-white' : 'text-gray-900'}`}>
                        {plan.name}
                      </h3>
                      <p className={`text-xs ${isHighlight ? 'text-gray-300' : 'text-gray-500'}`}>
                        {plan.description}
                      </p>
                    </div>

                    <div className='mb-6 border-b border-gray-200/20 pb-4'>
                      <span className={`text-2xl font-extrabold block ${isHighlight ? 'text-white' : 'text-gray-900'}`}>
                        {plan.price}
                      </span>
                      <span className={`text-xs font-medium ${isHighlight ? 'text-orange-400' : 'text-primary'}`}>
                        / {plan.duration}
                      </span>
                    </div>
                    ...
                    <ul className='space-y-3 mb-6 grow'>
                      {plan.features.map((feature, idx) => (
                        <li key={idx} className='flex items-start gap-2.5'>
                          <div className={`mt-0.5 min-w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${isHighlight ? 'bg-orange-500/20 text-orange-400' : 'bg-green-100 text-green-600'}`}>
                          ...
                          <div className='text-xs'>
                            ...
                            {feature.value !== true && (
                              <span className={`text-[10px] block mt-0.5 ${isHighlight ? 'text-gray-400' : 'text-gray-500'}`}>
                                {feature.value}
                              </span>
                            )}
                          </div>
            ...
            <div className='flex justify-center gap-2 mt-6'>
              {PRICING_PLANS.map((_, idx) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${idx === activePricingIndex ? 'bg-primary w-6' : 'bg-gray-300'
                    }`}
                />
              ))}
            </div>
